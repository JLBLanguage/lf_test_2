package com.liferay.test.action;
...
//    @Reference(
//            target = "(component.name=com.liferay.login.web.internal.portlet.com.liferay.test.action.CreateAccountMVCActionCommand)")
//
//    protected MVCActionCommand mvcActionCommand;

    private long[] findOrganizationIdsByRegisterCode(ActionRequest actionRequest,  long companyId){
        ...
        List<ExpandoValue> values = ExpandoValueLocalServiceUtil.getColumnValues(companyId, classNameId,
                ExpandoTableConstants.DEFAULT_TABLE_NAME, "registration-code",registrationCode, -1, -1);

        if(values != null){
            for (int i = 0; i < values.size(); i++) {
                long organizationId = values.get(i).getClassPK();
    ...
    private void addUserToOrganizations(long[] orgIds, long companyId, User user){
        if(orgIds.length <= 0){
            return;
        }
        for(int i=0; i < orgIds.length; i ++){
            _userLocalService.addOrganizationUser(orgIds[i], user);
        }
    }
    ...
    protected void addUser(
        ...
        long[] registrationOrgIds = findOrganizationIdsByRegisterCode(actionRequest, company.getCompanyId());
        if(registrationOrgIds.length <= 0){
        ...
        addUserToOrganizations(registrationOrgIds, company.getCompanyId(), user);
